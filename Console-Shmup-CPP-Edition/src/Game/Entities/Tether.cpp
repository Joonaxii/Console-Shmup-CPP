...
Tether::Tether() : Tether(0.0f, 1.0f, 4, 1.0f, 1.0f, 1.0f, 1.0f, WaveSpecifications(), 1.0f, 5.0f) { }

Tether::Tether(const float timeOffset, const float direction, const int tetherSize, const float speedGlobal, const float speedSeg, const float startSize, const float endSize, const WaveSpecifications& wave, const float bendPower, const float bendPowerMaxDist) : _timeOffset(timeOffset), _direction(direction), _tetherSize(tetherSize), _globalTimeSpeed(speedGlobal), _segTimeSpeed(speedSeg), _wave(wave), _bendPower(bendPower), _bendDist(bendPowerMaxDist) {
...
void Tether::update(const float delta) {
	...
	const Vector2 midPoint(Vector2::lerp(startPoint, endPoint, 0.5f) + Vector2::rotate(head, isLeft ? -90.0f : 90.0f) * _direction * _bendPower);

	const float h = _tetherSize * 0.5f;
	const float hL = h < 2.0f ? 1.0f : h - 1.0f;

	Vector2 heading((endPoint - startPoint).getNormalized());
	for (size_t i = 0; i < _tetherSize; i++) {
		const float n = i / l;
		...
		const float sinePoint(_wave.getValue(_timeGlobal, (_timeSeg + (n * 360.0f) + _timeOffset) * DEG_2_RAD) * n2 * _direction);

		const Vector2 pos(Vector2::lerp(linearPoint, bezPoint, bendPower) + (Vector2::rotate(heading, M_PI_2)) * sinePoint/*  Vector2::lerp(Vector2::zero, sinePoint, (1.0f - bendPower * 0.5f) * n2)*/);
		tr->setPosition(pos, true);

		if (i < _tetherSize - 1) {
			heading = (quadraticBezierCurve(startPoint, midPoint, endPoint, (i + 1) / l) - bezPoint).getNormalized();
		}
	}
}
