...
public:
    ...
    Tether();
    Tether(const float timeOffset, const float direction, const int tetherSize, const float speedGlobal, const float speedSeg, const float startSize, const float endSize, const WaveSpecifications& wave, const float bendPower, const float bendPowerMaxDist);
    ~Tether();
